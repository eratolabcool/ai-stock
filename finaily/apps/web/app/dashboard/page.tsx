...
const stocks = [
  ['NVIDIA', 'NVDA', '92', 'AI Semiconductor'],
  ['Microsoft', 'MSFT', '88', 'Cloud AI'],
  ['TSMC', 'TSM', '86', 'AI Infrastructure'],
  ['ASML', 'ASML', '85', 'Semiconductor Equipment'],
];
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white p-8 space-y-8">
      <header>
        ...
        <p className="mt-3 text-slate-400">AI companies, market themes and research signals.</p>
      </header>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        ...
        {stocks.map(([name,symbol,score,theme]) => <Link key={symbol} href={`/stocks/${symbol.toLowerCase()}`} className="block border-b border-slate-800 py-4">{name} ({symbol}) · {theme}<b className="float-right">{score}</b></Link>)}
      </section>
